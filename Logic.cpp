...
RandomLogic::RandomLogic() : generator(std::chrono::system_clock::now().time_since_epoch().count()) {}

bool RandomLogic::move(std::shared_ptr<Board> board, Player& player, SDL_Keycode event) {
	const int max = 3;
	int i = 0;
	int x = (generator() - 1) % max;
	int y = (generator() - 1) % max;
	while (board->getState(x, y) != State::FREE_OFF && board->getState(x, y) != State::FREE_ON) {
		x = (generator() - 1) % max;
		y = (generator() - 1) % max;
		if (++i > 1000)
			return false;
	}
	State s = player.getType() == PLAYER_TYPE::CIRCLE ? State::CIRCLE_OFF : State::CROSS_OFF;
	board->setState(x, y, s);
	return true;
}
bool HumanLogic::move(std::shared_ptr<Board> board, Player& player, SDL_Keycode event) {
	switch (event) {
...
State HumanLogic::switchState(const Player& player, const State s, const SDL_Keycode k) {
	switch (k) {
	case SDLK_UP:
	case SDLK_DOWN:
	case SDLK_RIGHT:
	...
	case SDLK_RETURN:
		switch (s) {
		case State::FREE_ON:
			return (player.getType() == PLAYER_TYPE::CIRCLE) ? State::CIRCLE_ON : State::CROSS_ON;
		case State::CROSS_OFF:
			return State::CROSS_ON;
		case State::CROSS_ON:
			return State::CROSS_ON;
