...
class Logic {
    public:
		virtual bool move(std::shared_ptr<Board> board, Player& player, SDL_Keycode event) = 0;
		virtual void reset() {};
		virtual  ~Logic() {};
};
...
class RandomLogic : public Logic {
	public:
		RandomLogic();
		virtual bool move(std::shared_ptr<Board> board, Player& player, SDL_Keycode event);
		virtual  ~RandomLogic() {};
	private:
